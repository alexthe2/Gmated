...
public class AdvancedGridLayout implements LayoutManager, java.io.Serializable {
    /*
     * serialVersionUID
     */
        ...
        synchronized (parent.getTreeLock()) {
            ...
            if (ltr) {
                for (int c = 0, x = insets.left + extraWidthAvailable; c < ncols ; c++, x += widthOnComponent + hgap) {
                    place(parent, insets, ncomponents, nrows, ncols, widthOnComponent, heightOnComponent, extraHeightAvailable, c, x);
                }
            } else {
    ...
    private void place(Container parent, Insets insets, int ncomponents, int nrows, int ncols, int widthOnComponent, int heightOnComponent, int extraHeightAvailable, int c, int x) {
        for (int r = 0, y = insets.top + extraHeightAvailable; r < nrows ; r++, y += heightOnComponent + vgap) {
            ...
            if (i < ncomponents) {
                ...
                if(comp instanceof IHotRescale) {
                    ((IHotRescale) comp).onRescale(widthOnComponent, heightOnComponent);
                }
            }
        }
    }
